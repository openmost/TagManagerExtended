    ...
    public function getParameters()
    {
        return array(
            $this->makeSetting('hjid', '', FieldConfig::TYPE_STRING, function (FieldConfig $field) {
                $field->title = Piwik::translate('TagManagerExtended_HotjarHJID');
                $field->description = Piwik::translate('TagManagerExtended_HotjarHJIDDescription');
                $field->customFieldComponent = self::FIELD_VARIABLE_COMPONENT;
                $field->validators[] = new NotEmpty();
            }),

            $this->makeSetting('hjsv', 6, FieldConfig::TYPE_INT, function (FieldConfig $field) {
                $field->title = Piwik::translate('TagManagerExtended_HotjarHJSV');
                $field->description = Piwik::translate('TagManagerExtended_HotjarHJSVDescription');
                $field->customFieldComponent = self::FIELD_VARIABLE_COMPONENT;
                $field->validators[] = new NotEmpty();
            }),
        );
